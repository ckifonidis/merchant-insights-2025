...
interface CustomersTimeSeriesChartProps {
  title: string;
  filters?: Filters;
}

/**
 * Smart Customers TimeSeries chart component for Dashboard
 ...
 */
const CustomersTimeSeriesChart: React.FC<CustomersTimeSeriesChartProps> = ({ title, filters }) => {
  const [timeline, setTimeline] = useState('daily');
  
  // Connect to store for customers_per_day metric
  const rawStoreData = useSelector(selectCustomersPerDay);
  const loading = useSelector(selectDataLoading);
  const errors = useSelector(selectDataErrors);
  
  // Check for specific loading/error states
  const isLoading = loading?.metrics || loading?.specificMetrics?.['customers_per_day'] || false;
  const error = errors?.metrics || errors?.specificMetrics?.['customers_per_day'] || null;
  ...
  const chartData = useMemo(() => {
    ...
    // Note: No competitor data for customers
    ...
    const currentYearData = merchantData.map((item, index) => ({
      ...
      competitorRevenue: 0 // No competitor data for customers
    }));
    ...
    const processedData = aggregatedCurrent.map((currentItem, index) => {
      ...
      return {
        ...
        competitor: 0, // No competitor data for customers
        merchantChange: parseFloat(merchantYoY.toFixed(1)),
        competitorChange: 0 // No competitor data for customers
      };
    });
