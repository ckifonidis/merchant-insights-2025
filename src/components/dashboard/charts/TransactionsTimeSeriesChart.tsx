...
interface TransactionsTimeSeriesChartProps {
  title: string;
  filters?: Filters;
}

/**
 * Smart Transactions TimeSeries chart component for Dashboard
 * Connects to Redux store, processes data, and passes to presentational component
 */
const TransactionsTimeSeriesChart: React.FC<TransactionsTimeSeriesChartProps> = ({ title, filters }) => {
  const [timeline, setTimeline] = useState('daily');
  
  // Connect to store for transactions_per_day metric
  const rawStoreData = useSelector(selectTransactionsPerDay);
  const loading = useSelector(selectDataLoading);
  const errors = useSelector(selectDataErrors);
  
  // Check for specific loading/error states
  const isLoading = loading?.metrics || loading?.specificMetrics?.['transactions_per_day'] || false;
  const error = errors?.metrics || errors?.specificMetrics?.['transactions_per_day'] || null;
